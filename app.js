...
function renderTable(carryIn, isFinished) {
  if (!els.scoreTable || !els.scoreTable.tBodies || !els.scoreTable.tBodies[0]) {
...
function renderRunningAudit() {
  const auditEl = document.getElementById('runningAudit');
  if (!auditEl) {
    console.error('runningAudit element not found!');
    return;
  }
  ...
  for (const h of Array.from(finishedHoles).sort((a, b) => a - b)) {
    ...
    if (scores.some(s => s.firstOn)) {
      baseAwarded++;
      if (carryIn.firstOn > 0) consumed.firstOn += carryIn.firstOn;
    }
    if (scores.some(s => s.closest)) {
      baseAwarded++;
      if (carryIn.closest > 0) consumed.closest += carryIn.closest;
    }
    if (scores.some(s => s.putt)) {
      baseAwarded++;
      if (carryIn.putt > 0) consumed.putt += carryIn.putt;
    }
    const par = courses[currentCourse].pars[idx];
    if (par === 3 && scores.some(s => s.firstOn)) {
      if (carryIn.greenie > 0) consumed.greenie += carryIn.greenie;
    }
  }

  const totalConsumed = consumed.firstOn + consumed.closest + consumed.putt + consumed.greenie;
  const carryOut = getCarryInForHole(HOLES + 1);
  const totalOpen = carryOut.firstOn + carryOut.closest + carryOut.putt + carryOut.greenie;

  const total = baseAwarded + totalConsumed + totalOpen;
  const balanced = total === totalBasePoints;

  const openLines = [];
  if (carryOut.firstOn) openLines.push(`FO: ${carryOut.firstOn}`);
  if (carryOut.closest) openLines.push(`CL: ${carryOut.closest}`);
  if (carryOut.putt) openLines.push(`P: ${carryOut.putt}`);
  if (carryOut.greenie) openLines.push(`GR: ${carryOut.greenie}`);

  auditEl.innerHTML = `
    <div class="audit-line ${balanced ? 'audit-good' : 'audit-bad'}">
      <strong>Round:</strong> ${baseAwarded} awarded + ${totalConsumed} consumed = ${baseAwarded + totalConsumed} / ${totalBasePoints}<br>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('#backToSetup').forEach(btn => {
